...
- `OMNI_TURN_URIS` plus `OMNI_TURN_REST_SECRET` for a self-hosted coturn server using short-lived REST credentials
...
The `/api/config` response now also reports `ice_source` so you can confirm whether the backend is using `metered_api`, `static_ice_json`, `turn_rest_env`, or another fallback path.
